    ...
    <PageEnter className="p-4 md:p-8 space-y-8 min-h-screen pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-4">
          <Link href="/issuers" className="flex items-center gap-2 text-sm font-bold text-[var(--shell-muted)] hover:text-[var(--foreground)] transition-colors group">
             <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
             Back to Admin Dashboard
          </Link>
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-violet-100 border border-violet-200 text-violet-700 dark:bg-violet-500/20 dark:border-violet-500/35 dark:text-violet-300 flex items-center justify-center shadow-sm">
               <Building2 size={28} />
            </div>
            <div>
        ...
        <div className="flex items-center gap-8">
          {tabs.map((tab) => (
            ...
            >
              {tab}
              {tab === 'Assets' && (
                <span className="bg-[var(--shell-active)] text-white text-[10px] px-1.5 py-0.5 rounded-full min-w-[1.25rem]">5</span>
              )}
              {activeTab === tab && (
                <motion.div
              ...
              <div className="lg:col-span-2 space-y-8">
                <div className="bg-[var(--shell-card)] p-8 rounded-[32px] border border-[var(--shell-card-border)] shadow-sm space-y-8">
                  <div>
                    ...
                    <p className="text-[var(--shell-muted)] leading-relaxed font-medium">{issuerData.bio}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                       ...
                       <div className="min-w-0">
                          ...
                          <p className="text-xs font-black text-[var(--foreground)] truncate">{issuerData.contact.email}</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                       ...
                       <div className="min-w-0">
                          ...
                          <p className="text-xs font-black text-[var(--foreground)]">{issuerData.contact.phone}</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                       <Globe size={16} className="text-[var(--shell-muted)]" />
                       <div className="min-w-0">
                          <p className="text-[10px] font-bold text-[var(--shell-muted)] uppercase ">Website</p>
                          <p className="text-xs font-black text-[var(--foreground)] truncate">{issuerData.contact.website}</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                       ...
                       <div className="min-w-0">
                          ...
                          <p className="text-xs font-black text-[var(--foreground)]">{issuerData.joined}</p>
                       </div>
                    </div>
                  </div>

                  <div className="space-y-4 pt-4">
                     <p className="text-[11px] font-black text-[var(--shell-muted)] uppercase ">Asset Summary</p>
                  ...
                  <ClientSizedChart className="h-64 w-full min-h-[16rem] min-w-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={issuerData.growthData}>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '16px', border: isDark ? '1px solid rgba(255,255,255,0.1)' : 'none', boxShadow: isDark ? '0 12px 40px rgba(0,0,0,0.45)' : '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 900, background: isDark ? '#0f172b' : '#ffffff', color: isDark ? '#f8fafc' : '#0f172a' }} />
                ...
                <div className="bg-[var(--shell-card)] p-8 rounded-[32px] border border-[var(--shell-card-border)] shadow-sm">
                  ...
                  <div className="space-y-4">
                    {issuerData.personnel.map((person) => (
                      <div key={person.name} className="flex items-center justify-between p-4 rounded-2xl bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                        <div>
                           <p className="text-sm font-black text-[var(--foreground)]">{person.name}</p>
                           <p className="text-[10px] font-bold text-[var(--shell-muted)] uppercase ">{person.role}</p>
                        </div>
        ...
        {activeTab === 'Assets' && (
          ...
          >
             <div className="p-8 border-b border-[var(--shell-card-border)]">
                <h2 className="text-lg font-black text-[var(--foreground)]">All Assets (5)</h2>
             </div>
             <div className="divide-y divide-[var(--shell-card-border)]">
                {issuerData.assets.map((asset) => (
                  <div key={asset.id} className="p-8 flex items-center justify-between hover:bg-[var(--shell-subtle)] transition-colors group">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <p className="text-[10px] font-bold text-[var(--shell-muted)] uppercase  mt-0.5">{asset.id} • {asset.type} • {asset.amount}</p>
                        </div>
                     </div>
                     <div className="flex items-center gap-12">
                        <div className="text-right hidden sm:block">
                           <p className="text-[10px] font-bold text-[var(--shell-muted)] uppercase  mb-0.5">Submitted</p>
                        ...
                        <div className="flex items-center gap-6">
                           ...
                           <ChevronRight size={18} className="text-[var(--shell-muted)] group-hover:text-[var(--shell-active)] transition-colors" />
                        </div>
                     </div>
                  </div>
                ))}
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {issuerData.financials.health.map((metric) => (
                    <div key={metric.label} className="p-6 rounded-[24px] bg-[var(--shell-subtle)] border border-[var(--shell-card-border)]">
                       ...
                       <span className={`text-[10px] font-black uppercase  ${metric.statusColor}`}>
                          {metric.status}
                       </span>
                    </div>
                  ))}
        ...
        {activeTab === 'Compliance' && (
          ...
          >
            {[
              ...
              <div key={item.label} className="bg-[var(--shell-card)] p-8 rounded-[32px] border border-[var(--shell-card-border)] shadow-sm dark:shadow-none space-y-8">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] font-bold text-[var(--shell-muted)] uppercase ">{item.desc}</p>
                    </div>
                 </div>
                 <div className="h-px w-full bg-[var(--shell-card-border)]" />
                 <div className="flex items-center justify-between">
                    <p className="text-[10px] font-bold text-[var(--shell-muted)]">Last Updated: {item.date}</p>
                    <span className="px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-black uppercase  border border-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:border-emerald-500/25">
                       {item.status}
                    </span>
                 </div>
